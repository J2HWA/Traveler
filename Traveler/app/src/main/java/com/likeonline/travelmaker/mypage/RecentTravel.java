...
import com.likeonline.travelmaker.place.PlaceModel;
...
public class RecentTravel extends AppCompatActivity {
    ...
    private FirebaseRecyclerAdapter<RecentTravel_Model, RecentTravelAdapter.ItemViewHolder> mPlaceAdapter;
    private final List<RecentTravel_Model> mPlaceModels = new ArrayList<>();
    private RecyclerView mPlace_list;
    float totalrating=0;
    public float reviewCount=0; //리뷰 갯수
    private Integer click=0; //조회수;
    private String uid;
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        mRecyclerView = (RecyclerView) findViewById(R.id.recent_travel_recyclerview);
        ...
        Toolbar board_toolbar = findViewById(R.id.recent_travel_toolbar);
        ...
        final DatabaseReference query = mReference.child("users").child(uid).child("place");
        FirebaseRecyclerOptions<RecentTravel_Model> options = new FirebaseRecyclerOptions.Builder<RecentTravel_Model>()
                .setQuery(query, RecentTravel_Model.class)
                .build(); // 어떤 데이터를 어디서 가져올것이며 어떤 형태의 데이터 클래스의 결과를 반환할건지 리사이클러뷰 어댑터 옵션에 넣음

        //어댑터랑 연결해서 리니어 레이아웃에 뿌림

        FirebaseDatabase.getInstance().getReference().child("users").child(uid).child("place").addValueEventListener(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                mPlaceModels.clear();

                for(DataSnapshot snapshot:dataSnapshot.getChildren()) {
                    RecentTravel_Model travelModel=snapshot.getValue(RecentTravel_Model.class);
                    mPlaceModels.add(travelModel);
                }
            }
        ...
        mPlaceAdapter = new FirebaseRecyclerAdapter<RecentTravel_Model, RecentTravelAdapter.ItemViewHolder>(options) {


            @NonNull
            @Override
            public RecentTravelAdapter.ItemViewHolder onCreateViewHolder(@NonNull ViewGroup viewGroup, int i) {
                View view = LayoutInflater.from(viewGroup.getContext())
                        .inflate(R.layout.item_place, viewGroup, false);
                return new RecentTravelAdapter.ItemViewHolder(view);
            }

            @Override
            protected void onBindViewHolder(final RecentTravelAdapter.ItemViewHolder holder, final int position, RecentTravel_Model model) {
    ...
    public boolean onOptionsItemSelected(final MenuItem item) {
        ...
        switch (item.getItemId()) {
            case android.R.id.home: // 뒤로가기 메뉴 클릭
                finish();
                return true;
            case R.id.action_delete: // 삭제하기 메뉴 클릭
                ...
                builder.setNegativeButton("예",
                        new DialogInterface.OnClickListener() {
                            public void onClick(DialogInterface dialog, int which) {
                               FirebaseDatabase.getInstance().getReference().child("users").child(uid).child("place").removeValue();
    ...
    public static class RecentTravelAdapter extends RecyclerView.Adapter<RecentTravelAdapter.ItemViewHolder> {
        RecentTravel travel = new RecentTravel();

        @NonNull
        @Override
        public RecentTravelAdapter.ItemViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
            // LayoutInflater를 이용하여 전 단계에서 만들었던 item.xml을 inflate 시킵니다.
            // return 인자는 ViewHolder 입니다.
            View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.item_place, parent, false);
            return new RecentTravel.RecentTravelAdapter.ItemViewHolder(view);
        }
        ...
        public int getItemCount() {
            ...
            return travel.mPlaceAdapter.getItemCount();
        }
